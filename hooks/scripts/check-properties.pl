...
# Hacked to check svn:keywords as well.
#
...
foreach my $path ( @files_added ) {
  ...
  if (not $mime_type) {
    ...
    if ($mime_type =~ /^text\// && not $eol_style) {
      push @errors, "$path : svn:mime-type=$mime_type but svn:eol-style is not set";
    }
    if ($mime_type && $mime_type =~ /^text\// && not $keywords) {
      push @errors, "$path : svn:keywords is not set to 'FreeBSD=%H'";
    }
  }
}
...
if (@errors) {
    ...
    Also, every non-binary file must have the svn:keywords property set.
    ...
    svn propset svn:keywords 'FreeBSD=%H' path/of/file

    You should set your ~/.subversion/config file up with auto-props
    so that this is done automatically.  See sample file at
      http://svn.freebsd.org/repo/svnadmin/misc/auto-props.txt
EOS
  exit 1;
} else {
  exit 0;
